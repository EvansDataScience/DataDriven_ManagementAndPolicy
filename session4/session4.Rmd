...
Here, you need to decide if you are satisfied with the data types you have. At least you see that the numeric columns are read as such by R. But, education represented as a number is a clear case where you need a category:
...
Now I can make replacements in that column, turning the diferent values into two groups:
...
Notice that the order of the replacing statements is important; if you run the next code, the last column will have only the value _2_. 
```{r, eval=TRUE}
#people$ageGroup[people$ageGroup>33]=1
#people$ageGroup[people$ageGroup<=33]=2
```
...
tapply(X=people$ages,
       INDEX=list(people$education,people$ageGroup),
       FUN=mean)
```
