...
Make a list...
```{r}
inslee=list('name'='Jay Robert Inslee',
            'dateOB'='9 February 1951',
            'placeOB'=list('city'='Seattle','state'='WA'),
            'marriageYear'=1972,
            'universities'=c('Stanford','UW','Willamette'))
```


... in a way you can answer these questions:

* How old is he? (compute it from his birthday)
```{r}
library(lubridate)

inslee$dob=as.Date(inslee$dob,format="%d %B %Y")
time_length(interval(inslee$dob,Sys.Date()),"years")
```

* How long has he been married?

```{r}
2019-inslee$marriageYear
```

* How many Universities has he attended?

```{r}
length(inslee$universities)
```

* In what state was he born?

```{r}
inslee$placeOB$state
```
